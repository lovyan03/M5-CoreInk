...
#define INK_SPI_FREQUENCY  40000000 
...
GPIO.out_w1tc = (1 << INK_SPI_CS);
#define CS_WRITE_H \
while (READ_PERI_REG(SPI_CMD_REG(SPI_PORT))&SPI_USR); \
GPIO.out_w1ts = (1 << INK_SPI_CS);

#define DC_WRITE_L \
while (READ_PERI_REG(SPI_CMD_REG(SPI_PORT))&SPI_USR); \
GPIO.out_w1tc = (1 << INK_SPI_DC);
#define DC_WRITE_H \
while (READ_PERI_REG(SPI_CMD_REG(SPI_PORT))&SPI_USR); \
GPIO.out_w1ts = (1 << INK_SPI_DC);
...
private:
    ...
    int writeData( uint8_t data , uint16_t len );
    int writeDataArray( const uint8_t* data , uint16_t len );
    int waitbusy( uint16_t time);
    

private:

    int32_t _transaction_count = 0;
    bool _isInit = false;
